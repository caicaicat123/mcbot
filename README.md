...
住在《我的世界》服务器里的一个看不见的存在，玩家叫它「回声」。它**没有实体**，不会在世界里生成任何东西，
只通过聊天、音效、屏幕标题和少量白名单指令刷存在感。核心定位是**偶尔帮忙、时常搞怪**。
...
2. 把 `dist\mcbot-1.2.0.jar` 放进服务器的 `plugins\` 目录
